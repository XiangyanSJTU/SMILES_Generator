...
def add_task(smiles_list, category, remark=None):
    if session.query(Task).filter(Task.smiles_list == json.dumps(smiles_list)).first() is not None:
        return
    new_info = Task()
    new_info.smiles_list = json.dumps(smiles_list)
    new_info.n_heavy = get_heavy_atom_numbers(smiles_list[0])
    new_info.category = category
    new_info.molecule_number = len(smiles_list)
    new_info.charge = get_charge(smiles_list[0])
...
def add_atom(smiles_list, bond_order, category, atom_num, growing_atom=6, remark=None, stable_check=False):
    new_smiles_list = []
...
def sub_double_bond(smiles_list, category=None, smiles_list_unstable=None):
    ...
    for smiles in smiles_list:
        ...
        for bond in rdk_mol.GetBonds():
            if bond.GetBondTypeAsDouble() == 1 and bond.GetBeginAtom().GetImplicitValence() != 0 and \
                ...
                new_smiles = get_canonical_smiles(Chem.MolToSmiles(rdk_mol))
    ...
    if new_smiles_list_unstable != []:
        ...
        new_info.n_heavy = get_heavy_atom_numbers(new_smiles_list_unstable[0])
        new_info.category = category
...
def growing_from_smiles(n_heavy_cutoff, smiles, category):
    ...
    n = get_heavy_atom_numbers(smiles)
